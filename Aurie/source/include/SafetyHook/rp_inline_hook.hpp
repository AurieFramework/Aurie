...
/// @brief A register-preserving inline hook.
class SAFETYHOOK_API RpInlineHook final {
public:
    ...
    /// @brief Error type for RpInlineHook.
    struct Error {
        ...
        enum : uint8_t {
            ...
            UNSUPPORTED_ARCHITECTURE_FOR_RP_HOOK   ///< The current architecture is not supported.
        } type;
        ...
        /// @brief Create a UNSUPPORTED_ARCHITECTURE_FOR_RP_HOOK error.
        /// @param ip The IP of the problematic instruction.
        /// @return The new UNSUPPORTED_ARCHITECTURE_FOR_RP_HOOK error.
        [[nodiscard]] static Error unsupported_architecture_for_rp_hook(uint8_t* ip) {
            Error error{};
            error.type = UNSUPPORTED_ARCHITECTURE_FOR_RP_HOOK;
            error.ip = ip;
            return error;
        }
    ...
    /// @return The RpInlineHook or an RpInlineHook::Error if an error occurred.
    ...
    [[nodiscard]] static std::expected<RpInlineHook, Error> create(
    ...
    [[nodiscard]] static std::expected<RpInlineHook, Error> create(T target, U destination, Flags flags = Default) {
    ...
    RpInlineHook() = default;
    RpInlineHook(const RpInlineHook&) = delete;
    RpInlineHook(RpInlineHook&& other) noexcept;
    RpInlineHook& operator=(const RpInlineHook&) = delete;
    RpInlineHook& operator=(RpInlineHook&& other) noexcept;
    ~RpInlineHook();
    ...
    /// @brief Gets the register state for the last call.
    [[nodiscard]] const RegisterContext& register_state() const;
...
private:
    ...
    std::expected<void, Error> write_rp_code(void* data);
